  ...
  factory WebGLRendererParameters.fromMap(Map<String, dynamic> map) {
    return WebGLRendererParameters(
      ...
      failIfMajorPerformanceCaveat:
          map["failIfMajorPerformanceCaveat"] ?? false,
...
class WebGLRenderer {
      ...
      if (isIntegerFormat) {
        ...
        if (isUnsignedType) {
          ...
          _gl.clearBufferuiv(
            WebGL.COLOR,
            0,
            uintClearColor.buffer.asByteData().getUint32(0),
          );
        } else {
          ...
          _gl.clearBufferiv(
            WebGL.COLOR,
            0,
            intClearColor.buffer.asByteData().getInt32(0),
          );
        }
      } else {
  ...
  void renderBufferDirect(
    ...
    scene ??= _emptyScene;
    final frontFaceCW =
        (object is Mesh && object.matrixWorld.determinant() < 0);

    WebGLProgram program = setProgram(
      camera,
      scene,
      geometry,
      material,
    ...
    int drawEnd =
        ((drawRange['start'] ?? 0) + (drawRange['count'] ?? 0)) * rangeFactor;
    ...
    if (object is BatchedMesh) {
      if (object.multiDrawInstances != null) {
        renderer.renderMultiDrawInstances(
          object.multiDrawStarts,
          object.multiDrawCounts,
          object.multiDrawCount,
          object.multiDrawInstances!,
        );
      } else {
        if (!extensions.get('WEBGL_multi_draw')) {
          ...
          final bytesPerElement = index != null
              ? attributes.get(index).bytesPerElement
              : 1;
          final uniforms = properties
              .get(material)['currentProgram']
              .getUniforms();
    ...
    if (object is InstancedMesh) {
      ...
      final instanceCount = math.min(
        geometry.instanceCount!,
        geometry.maxInstanceCount ?? 0,
      );
  ...
  void render(Object3D scene, Camera camera) {
    ...
    currentRenderState = renderStates.get(
      scene,
      renderCallDepth: renderStateStack.length,
    );
    currentRenderState!.init(camera);

    renderStateStack.add(currentRenderState!);

    projScreenMatrix.multiply2(
  ...
  void renderTransmissionPass(
    List<RenderItem> opaqueObjects,
    List<RenderItem> transmissiveObjects,
    Object3D scene,
    Camera camera,
  ) {
    ...
    if (currentRenderState?.state.transmissionRenderTarget[camera.id] == null ||
        (activeViewport.w.toInt() != transmissionRenderTarget?.height ||
            activeViewport.z.toInt() != transmissionRenderTarget?.width)) {
    ...
    transmissionRenderTarget!.setSize(
      (activeViewport.z * transmissionResolutionScale).toInt(),
    ...
    if (_clippingEnabled == true)
      clipping.setGlobalState(clippingPlanes, camera);
  ...
  void renderObject(
    ...
    object.modelViewMatrix.multiply2(
      camera.matrixWorldInverse,
      object.matrixWorld,
    );
  ...
  WebGLProgram? getProgram(
    ...
    final parameters = programCache.getParameters(
      material,
      lights.state,
      shadowsArray,
      scene,
  ...
  List getUniformList(Map materialProperties) {
    if (materialProperties['uniformsList'] == null) {
      final progUniforms =
          (materialProperties['currentProgram'] as WebGLProgram).getUniforms();
  ...
  WebGLProgram setProgram(
    ...
    final fog = scene.fog;
    final environment = material is MeshStandardMaterial
        ? scene.environment
        : null;
    final colorSpace = (_currentRenderTarget == null)
        ? outputColorSpace
        : (_currentRenderTarget?.isXRRenderTarget == true
              ? _currentRenderTarget?.texture.colorSpace
              : LinearSRGBColorSpace);
    final envMap = (material is MeshStandardMaterial
        ? cubeuvmaps.get(material.envMap ?? environment)
        : cubemaps.get(material.envMap ?? environment));
    final vertexAlphas =
        material.vertexColors &&
        geometry?.attributes['color'] != null &&
        geometry?.attributes['color'].itemSize == 4;
    final vertexTangents =
        geometry?.attributes['tangent'] != null &&
        (material.normalMap != null ||
            (material is MeshPhysicalMaterial && material.anisotropy > 0));
    ...
    final morphAttribute =
        geometry?.morphAttributes['position'] ??
        geometry?.morphAttributes['normal'] ??
        geometry?.morphAttributes['color'];
    final morphTargetsCount = (morphAttribute != null)
    ...
    if (refreshProgram || _currentCamera != camera) {
      ...
      final reverseDepthBuffer = (state.buffers['depth'] as DepthBuffer)
          .getReversed();
    ...
    if (material is MeshGouraudMaterial && material.envMap != null) {
      ...
      mUniformS['flipEnvMap']['value'] =
          (envMap is CubeTexture && envMap.isRenderTargetTexture == false)
          ? -1
          : 1;
    }
  ...
  void setRenderTargetTextures(
    ...
    if (!(renderTargetProperties["__autoAllocateDepthBuffer"] == true)) {
      if (extensions.has('WEBGL_multisampled_render_to_texture') == true) {
        console.warning(
          'WebGLRenderer: extension was disabled because an external texture was provided',
        );
  ...
  void setRenderTarget(
    ...
    if (renderTarget != null) {
      ...
      if (texture is Data3DTexture ||
          texture is DataArrayTexture ||
          texture is CompressedArrayTexture) {
        isRenderTarget3D = true;
      }

      final webglFramebuffer = properties.get(
        renderTarget,
      )["__webglFramebuffer"];
    ...
    final framebufferBound = state.bindFramebuffer(
      WebGL.FRAMEBUFFER,
      framebuffer,
    );
    ...
    if (isCube) {
      ...
      _gl.framebufferTexture2D(
        ...
        WebGL.TEXTURE_CUBE_MAP_POSITIVE_X + activeCubeFace,
  ...
  void readRenderTargetPixels(
    ...
    dynamic framebuffer = properties.get(
      renderTarget,
    )["__webglFramebuffer"]; //can be Map or int
      ...
      try {
        ...
        if (textureFormat != RGBAFormat &&
            utils.convert(textureFormat) !=
                _gl.getParameter(WebGL.IMPLEMENTATION_COLOR_READ_FORMAT)) {
          console.warning(
        ...
        if (textureType != UnsignedByteType &&
            (kIsWeb &&
                utils.convert(textureType) !=
                    _gl.getParameter(
                      WebGL.IMPLEMENTATION_COLOR_READ_TYPE,
        ...
        if ((x >= 0 && x <= (renderTarget.width - width)) &&
          ...
          _gl.readPixels(
            ...
            utils.convert(textureFormat),
            utils.convert(textureType),
            kIsWeb ? buffer.data : buffer,
          );
        }
      } finally {
        final framebuffer = (_currentRenderTarget != null)
  ...
  void copyTextureToTexture(
    ...
    int dstX, dstY, dstZ;
    final image = srcTexture is CompressedTexture
        ? srcTexture.mipmaps[dstLevel]
        : srcTexture.image;
    if (srcRegion != null) {
      width = srcRegion.max.x - srcRegion.min.x;
      height = srcRegion.max.y - srcRegion.min.y;
      depth = srcRegion is BoundingBox
          ? (srcRegion.max.z - srcRegion.min.z).toInt()
          : 1;
    ...
    if (dstTexture is Data3DTexture) {
      ...
      glTarget = WebGL.TEXTURE_3D;
        ...
        dstTexture is CompressedArrayTexture) {
    ...
    final currentUnpackImageHeight = _gl.getParameter(
      WebGL.UNPACK_IMAGE_HEIGHT,
    );
    ...
    final isSrc3D =
        srcTexture is DataArrayTexture || srcTexture is Data3DTexture;
    final isDst3D =
        dstTexture is DataArrayTexture || dstTexture is Data3DTexture;
    if (srcTexture.isDepthTexture) {
      final srcTextureProperties = properties.get(srcTexture);
      final dstTextureProperties = properties.get(dstTexture);
      final srcRenderTargetProperties = properties.get(
        srcTextureProperties['__renderTarget'],
      );
      final dstRenderTargetProperties = properties.get(
        dstTextureProperties['__renderTarget'],
      );
      state.bindFramebuffer(
        WebGL.READ_FRAMEBUFFER,
        srcRenderTargetProperties['__webglFramebuffer'],
      );
      state.bindFramebuffer(
        WebGL.DRAW_FRAMEBUFFER,
  ...
  set outputColorSpace(String colorSpace) {
    ...
    gl.drawingBufferColorSpace = colorSpace == DisplayP3ColorSpace
        ? 'display-p3'
        : 'srgb';
    gl.unpackColorSpace =
